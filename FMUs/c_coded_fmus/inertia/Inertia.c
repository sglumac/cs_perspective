...
fmi2Status fmi2GetReal(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, fmi2Real value[])
{
	...
	for (i = 0; i < nvr; i++)
	{
		...
		{
		case 0:
			value[i] = Inertia->torque;
			break;
		case 1:
			value[i] = Inertia->velocity;
			break;
		case 2:
			value[i] = Inertia->J;
			break;
	...
	const fmi2CallbackFunctions* functions, fmi2Boolean visible, fmi2Boolean loggingOn)
{
	...
	else
	{
		...
		Component* Inertia = malloc(sizeof(Component));
		//initialize time
	...
	fmi2Boolean noSetFMUStatePriorToCurrentPoint)
{
	...
	else  if (abs(Inertia->time - currentCommunicationPoint) > 0.000000001) // check time integrity, should not be modified
	{
		return fmi2Error;
	}
	else
	{
		...
		// model time update
		Inertia->time += communicationStepSize;

		// model equations should be implemented here
		Inertia->velocity = Inertia->velocity + communicationStepSize / Inertia->J * ( -Inertia->d*Inertia->velocity + Inertia->torque);
